...
## install Node Exporter container  on remote  servers

look ansible  role  in Infrastructure/Ansible
...
server {
    listen 80;
    server_name [uygulaman.com veya DMZ IP];

    location / {
        proxy_pass http://192.168.100.x:18080; # APP Sunucusu IP'si
...
```bash
dnf install -y mariadb-server # Depoları güncelle ve MariaDB'yi kur
systemctl enable --now mariadb # Servisi başlat ve açılışta çalışacak şekilde ayarla
mariadb-secure-installation # determine root  pasword

```
for Network isolation, set 
```vim
[mysqld]
bind-address = [DB_SERVER_VLAN_IP]
```
...
```bash
firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="DB_SERVER_IP" port protocol="tcp" port="3306" accept'
firewall-cmd --reload
```
Construct user, database  and connection to app server
```bash
mysql -u root -p # enter mariadb as root

CREATE DATABASE IF NOT EXISTS corp_app_db; # construct test database
 
CREATE USER 'app_user'@'APP_SERVER_IP' IDENTIFIED BY 'asd'; # a user that can connect only connect from app-swerver

GRANT ALL PRIVILEGES ON corp_app_db.* TO 'app_user'@'AP_SERVEER_IP'; # allow this user only for this database
...
# APP SERVER
construct a simple app file app.py 
```py
...
def check_db():
    try:
        conn = mariadb.connect(
            ...
            host="[DB_ZONE_IP]",
            port=3306,
            database="corp_app_db"
        )
        return "<h1>Status: Connected to MariaDB Successfully!</h1>"
    except mariadb.Error as e:
        return f"<h1>Status: Connection Failed! Error: {e}</h1>"
...
```bash
sudo docker build -t my-secure-app .

# when proxy used
sudo docker build --no-cache --build-arg http_proxy=http://192.168.100.240:3128 --build-arg https_proxy=http://192.168.100.240:3128 --network host -t my-secure-app .

sudo docker run -d --restart always --name web-app -p 18080:18080 my-secure-app 
```
...
# BACKUP
## pfSense: 
The config.xml file for full network recovery.
Diagnostics > Backup & Restore  select all and download. 

## FreeIPA: 
Full snapshots of the LDAP directory and Kerberos keys using ipa-backup.
sudo ipa-backup

## Databases: 
Encrypted SQL dumps from the DB Zone.